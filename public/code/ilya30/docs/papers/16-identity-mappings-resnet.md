# 16. Identity Mappings：让残差主干真正成为无障碍信息高速路
...
[`src/ilya30/vision.py`](../../src/ilya30/vision.py) 的残差示例区分 post-activation 与 clean identity，并计算连续 scalar shortcut 的梯度乘积。代码刻意不复刻 1001 层训练；完整历史实现可看作者仓库。Transformer 中与 clean residual path 相似的 pre-norm 现象，可对照后来的 [LayerNorm 位置分析](https://arxiv.org/abs/2002.04745)，但两种架构不能直接等同。
