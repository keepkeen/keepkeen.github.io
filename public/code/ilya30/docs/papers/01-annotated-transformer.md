# 01. The Annotated Transformer：把论文变成可执行的解释
...
[Attention Is All You Need](14-attention-is-all-you-need.md) 给出完整模型和实验，却以研究论文的密度组织内容。2017 年的官方 Tensor2Tensor 实现面向研究系统，阅读成本高。Sasha Rush 把模型压到约 400 行教学代码，并让正文、公式和相邻代码共同出现。这个做法后来成为“可执行论文”的代表案例，也正式发表于 [ACL 2018 Workshop for NLP Open Source Software](https://aclanthology.org/W18-2509/)。
...
本项目在 [`src/ilya30/attention.py`](../../src/ilya30/attention.py) 独立实现缩放点积注意力、因果掩码、多头拆分和正弦位置编码；章节 14 再把这些部件放回原论文的完整论证。
