# 23. Scaling Laws：把“做多大、喂多少数据、练多久”变成可拟合的工程问题
...
[`src/ilya30/systems.py`](../../src/ilya30/systems.py) 实现带可选不可约 loss floor 的 log-space power-law 拟合、`6ND` 估算和固定预算网格搜索。测试用合成数据恢复指数，并展示改变 floor/拟合区间会系统改变外推。输出附残差，避免只报一条直线。
