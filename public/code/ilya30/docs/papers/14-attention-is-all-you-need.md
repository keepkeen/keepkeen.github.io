# 14. Attention Is All You Need：把序列计算改造成全局可寻址的并行图
...
[`src/ilya30/attention.py`](../../src/ilya30/attention.py) 独立实现稳定 softmax、mask、scaled dot-product、多头拆合和正弦位置编码。测试包括：每行权重和为 1；因果位置看不到未来；多头拆合不改变张量元素；所有 key 被 mask 时明确报错。完整教学实现见第 1 篇 [Annotated Transformer](01-annotated-transformer.md)。

## 它从哪里来，其他路线如何解决

[Bahdanau attention](15-bahdanau-attention.md) 让 RNN decoder 对 encoder 状态做可微软寻址，打破固定向量瓶颈；[Luong attention](https://arxiv.org/abs/1508.04025) 系统比较点积/双线性对齐；[Memory Networks](https://arxiv.org/abs/1410.3916) 和 [NTM](21-neural-turing-machines.md) 把注意力解释为外部记忆寻址。Transformer 的突破是让序列内部也全部依靠这种寻址。
