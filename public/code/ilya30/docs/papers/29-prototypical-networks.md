# 29. Prototypical Networks：让少样本分类退化成“离哪个中心最近”
...
没有证据表明 Ilya 点名了这篇论文。它被放在编辑补全中的理由是：三篇候选需要覆盖元学习的不同“快适应载体”——[MANN](28-memory-augmented-meta-learning.md) 改状态/记忆，本文即时估计类别表示，[MAML](30-maml.md) 改模型参数。ProtoNet 又把 Matching Networks 的实例注意力压缩成一个可以推导、可以画图、可以复用的统计量，适合作为度量元学习的核心入口。
...
本项目在 [`meta.py`](../../src/ilya30/meta.py) 提供 prototype、距离 logits 和概率的 NumPy 实现，并用“prototype logits 与线性 logits 只差 query 常数”做单元测试。
