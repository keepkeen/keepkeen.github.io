# 27. CS231n（2016）：从梯度检查到训练 ImageNet 网络的实践底座
...
本项目不复制课程作业答案。核心 NumPy 机制分别在 [`vision.py`](../../src/ilya30/vision.py)、[`sequence.py`](../../src/ilya30/sequence.py) 和 [`attention.py`](../../src/ilya30/attention.py)：softmax/cross-entropy、gradient check、卷积/空洞卷积、残差、RNN/LSTM 与 attention。tests 将解析梯度和数值梯度对照，并检查卷积尺寸。
...
[AlexNet](08-alexnet.md) 是课程历史转折点；[ResNet](11-resnet.md) 解释如何继续加深；[dilated convolution](12-dilated-convolutions.md) 改造 dense prediction 感受野；RNN/LSTM 和 attention 把视觉扩到 captioning。CS229/统计学习提供更广数学底座，Goodfellow–Bengio–Courville 的 *Deep Learning* 给系统教材，CS224n 聚焦语言。
