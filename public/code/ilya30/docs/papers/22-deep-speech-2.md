# 22. Deep Speech 2：端到端不仅是一个损失函数，而是一整套规模化系统
...
[`src/ilya30/ctc.py`](../../src/ilya30/ctc.py) 实现 log-space CTC forward，正确处理 blank、相邻重复字符和 impossible alignment。测试将短序列 DP 与路径穷举对照，并检查重复标签至少需要中间 blank。完整 ASR 训练应使用框架的高性能 CTC kernel。
