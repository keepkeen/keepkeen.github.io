---
title: "13. Neural Message Passing：用一个框架统一分子图网络"
description: "MPNN 把一大批图神经网络归纳为两个阶段：节点沿边发送并聚合消息，反复更新局部状态；最后用对节点排列不敏感的 readout 得到整图预测。它既是具体分子性质模型，也是至今仍通用的 GNN 接口语言。"
date: 2026-08-23
tags:
  - deep-learning
  - paper-reading
...
[`src/ilya30/graphs.py`](https://github.com/keepkeen/keepkeen.github.io/blob/main/public/code/ilya30/src/ilya30/graphs.py) 实现单步 MPNN 和 invariant readout。测试会一致地置换节点与边端点索引，验证图级输出相同，并验证 `T` 步只能越过 `T` 条边。这比复刻完整 QM9 训练更能暴露机制；真实分子建模可使用 [PyTorch Geometric MessagePassing](https://pytorch-geometric.readthedocs.io/en/latest/generated/torch_geometric.nn.conv.MessagePassing.html)。
