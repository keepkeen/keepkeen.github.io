...
> **2026-09-08 校准**：根据已完成的验收记录继续。下面的 D 编号按连续执行计算；有欠账时回到最早未通过的关键验收。原表中的 7—8 月日期是历史排期，当前以 D1–D30 相对日为准。

| 你的真实状态 | 今天做什么 | 新终点 |
|---|---|---|
| 原 D1–D30 验收项完成 ≥80%，且 §5 至少 5/7 通过 | 进入下方“维持模式”，只修弱项和做公司定向 | 不设假终点；持续到 offer |
| 8.23 开始并持续执行 | 今天是 **D17**；完成本日任务并复测 W2 的 DPO/GRPO loss | 2026-09-21 为 D30 |
| 8.29 开始并持续执行 | 今天是 **D11**；继续二分答案/差分、MoE 与后训练知识 | 2026-09-27 为 D30 |
| 今天才系统开始 | 今天是 **D1**；先保 P0、每天留投递时间 | 2026-10-07 为 D30，9 月笔面照常参加 |

今天先核验未来 7 天的真实笔面通知和[招聘窗口](/blog/recruit-window-refresh-log/)。vivo 2027 FAQ 写明 9 月 15 日 12:00 截止、15:00 研发岗统一笔试，只有一次笔试机会；确认报考者把该时间留空。其他公司的场次以本人通知为准。

### 9 月 8—14 日执行表（替换当天同类任务，不叠加课时）

这是一张公司定向复习表；基础未过者先做对应 D 日 P0，把下表作为题目替换。每天算法 60–90 分钟、手写 45–60 分钟、口述/项目 60 分钟，剩余时间用于主线学习与投递。表中力扣题是训练映射，不宣称是本周公司原题。

| 日期 | 力扣/算法 | 模型手写或 AI Coding | 面经回答与验收 |
|---|---|---|---|
| 9.08 | LC 3、560：滑窗与前缀和，解释负数为何破坏滑窗条件 | stable softmax + MHA mask | 90 秒讲项目：任务、数据、baseline、个人贡献、可核验指标 |
| 9.09 | LC 215；受限数字构造：严格小于、回退、前导零 | top-k/top-p + 固定 seed；说明抽样语义 | SFT/DPO/GRPO 的数据、策略与损失分别是什么 |
| 9.10 | LC 307 + 两两绝对差维护：count/sum 两棵 Fenwick | DPO loss，检查 chosen/rejected、reference、beta | 算法正确性用暴力小样本对拍；解释复杂度瓶颈 |
| 9.11 | LC 200、207：DFS/BFS、拓扑环检测 | GRPO advantage/mask，测试同分组、全 padding | πθ、πold、πref 区别；工具返回文本为什么通常不计策略 loss |
| 9.12 | 90–120 分钟目标公司模拟；彩门换向可替换模拟题 | NumPy 逻辑回归：JSON→训练→概率输出 | 记录独立完成率、耗时、泄漏检查；自拟协议见 AI Coding 补丁 §13 |
| 9.13 | LC 300、72：DP 状态与转移；24h 错题复写 | 训练/验证切分，预处理只 fit 训练集 | Agent 的停止、重试、权限、轨迹重放；评测是否真的闭环 |
| 9.14 | LC 102、146；只回炉旧错题 | 随机抽 MHA/DPO/GRPO 一件限时盲写 | 45 分钟 mock；核对 9.15 笔试通知和设备，晚间降载 |

当天有真实笔面时用它替代模拟，24 小时内补记错因与复写日期。RL 专题的 60 道力扣与本表重叠题只做一次；总计划和 RL 计划共享时间预算。
