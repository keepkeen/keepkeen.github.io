...
seriesOrder: 13
---
> 建立日期：2026-08-13；最后增量：2026-09-08。§1 对接 CV/检测背景；§5 信用粒度、§6 可学习性与 judge 校准；§7 新增九月论文的阶段归因、奖励/切分审计与 Agent 约束案例。新预印本与实录题分开使用，先完成基础章节再阅读扩展。
...
## 7. 2026-09-08 增量：用新论文练习实验判断

以下三篇均在 9 月 2—4 日首发。它们作为基础知识的案例阅读，暂不加入 79 张必背卡的评分总数；作者实验尚未在本仓库复现。阅读顺序建议 7.1 → 7.2 → 7.3，每项先回答问题再看论文。

### 7.1 竞赛代码后训练：提升究竟发生在哪一阶段？

[Post-Training Language Models for Gold-Medal Performance in Coding Competitions](https://arxiv.org/html/2609.02849v2)（9.02 首发、9.04 v2）的 Nano 模型在 IOI 2025 上，单次得分约为基座 130、SFT 后 280、RL 后 291；使用 GenCorrect 推理时多轮反馈后约 468。GRPO 的终局奖励来自程序执行，GenCorrect 则在推理时生成候选、选择并利用反馈修正。Ultra 没有做本文的代码专项 RL，但初始化含此前 RLVR，不能说从未经过 RL。IOI 2026 测试是非官方评测，不是官方参赛名次。

**怎样理解：** 参数更新改变的是策略；采样次数、工具反馈与选择器改变的是调用策略的方式。两者都可能有用，但需要分别记录。上面的 291 和 468 使用不同推理设置，不能把差值称为 RL 训练收益。

**面试回答框架：**“我会先固定提示词、采样温度、token/时间预算、测试集与选择规则，比较 base/SFT/RL；再冻结同一 checkpoint，比较单次、best-of-N、多轮反馈。报告成功率同时报告总生成 token、执行次数、延迟与成本。多轮反馈使用的测试也要与最终隐藏验收区分。”

**练习规格（尚未运行）：** 做一张 `checkpoint × inference_mode` 结果表，保留没有跑的空格。若 RL 优势只在更大搜索预算下出现，只能说组合方案有效；若相同预算下仍提升，才有相应条件下的训练收益证据。该练习复用受控比较原则，不是新的训练算法。

### 7.2 文本简化：确定性奖励也可能偏离目标

[Catalan text simplification with RL](https://arxiv.org/html/2609.04823v1)（9.04 首发）用 GRPO 优化 SARI；作者观察到冗长、不停止的输出可能得高分，并引入长度/复制惩罚。论文明确承认按“原句—参考句对”切分导致同一原句跨集合的风险，且每种语言只有一次训练。英语训练到加泰语 iDEM 的 SARI 为 43.31→44.50，不能据此得出普遍跨语言结论。

**先问目标：** 文本简化希望保留事实、降低阅读难度；SARI 是代理指标。可计算只保证打分过程可执行，不能保证它等价于人的目标。训练 reward 上升时应同时查看长度、重复、遗漏和语义保真。

**切分教学例子：** 输入原句 A 有参考答案 A1、A2、A3。若把 `(A,A1)` 放训练、`(A,A2)` 放测试，即使字符串 pair 不重复，模型已经见过测试输入。应先按原句/用户/文档或题目族分组，再划分 train/dev/test；检索语料、合成样本、不同参考也按任务边界检查。

**面试回答框架：**“先确认指标实现、数据切分和终止条件；用固定模型构造冗长、复制、删掉关键信息等反例，检查打分排序。修复泄漏后再评估基线和训练模型。只有评分目标仍不合理时，才单独提出奖励修改并做对照。”这样可以区分评测问题与算法问题，避免看到坏输出就连续叠惩罚项。

### 7.3 Agent 约束：低价格能抵消日期错误吗？

[First Things First](https://arxiv.org/html/2609.05224v1)（9.04 首发）区分必须条件、可选偏好和无可行解，用 GRPO 结合格式、答案与需求分类奖励。Qwen2.5-VL-7B 总准确率 39.78%→55.80%，但无解子集 44.26%→42.62%。答案比较使用多模态模型，因此不是纯规则 RLVR；图文需求推理结果也不能直接代表长程工具 Agent 的可靠性。

**教学例子：** 用户要求“9 月 15 日出发，预算不超过 800，尽量靠窗”。日期和预算是必须条件，靠窗是偏好。候选 A 日期错误但便宜且靠窗，候选 B 日期预算都满足但不靠窗。业务应优先 B；加权总分若给便宜/靠窗过高权重，可能错误选择 A。

用集合表达更清楚：先构造满足必须条件的可行集合，再在集合内按偏好排序；为空时应报告不可行并请求用户调整条件。此处是业务决策规格，不能据此声称论文的软奖励训练保证了硬约束。

**面试验收：** 同时报告总体成功率、硬条件违反率、无解识别率、可行任务误拒率。比较随机无解样本与最接近可行的困难负例。若平均分涨、无解识别下降，就需要说明收益发生在哪些子集；不能只报总分。
