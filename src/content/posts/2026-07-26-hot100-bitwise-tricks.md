...
> 代码约定同主篇:Python 函数是算法核心,提交 LeetCode 时仍需按题目签名放进 `class Solution` 并改成对应的 camelCase 方法名。
...
| 22 | 原地哈希 | 答案落在 [1,n+1],只归位 [1,n] 内的值 | 41 |
...
def bit_tricks(x, i):
    ...
    count_one = x.bit_count()         # 本模板约定 x >= 0
...
- `a^a=0` 别写成 `a&a`。
- 本篇「统计 1」默认输入是非负整数;若题目给 w 位有符号数,先用 `x & ((1 << w) - 1)` 取得其 w 位无符号表示再 `bit_count()`,不能把 Python 负数直接当固定宽度补码。
...
**识别信号**
- 数组长度 n,问「缺失的第一个正数」:答案必在 `[1,n+1]`,因此只需归位数组中落在 `[1,n]` 的值;原数组可以包含负数、0 和大于 n 的数。
...
- **螺旋 = 四边界收缩模拟**;
- **有序矩阵查找 = 从右上角走 Z 字**(当前值太大就左移、太小就下移)。
...
**复杂度**:旋转/置零/螺旋时间 O(m·n),Z 字查找 O(m+n)。旋转、置零、查找的辅助空间 O(1);螺旋遍历除返回结果外辅助空间 O(1),返回列表本身占 O(m·n)。
...
| 统计 1 的个数(非负 x) | `x.bit_count()`;w 位有符号数先 `x &= (1 << w) - 1` |
| 判奇偶 | `x & 1` |
| Python 交换 | `a, b = b, a` |
...
长度 n,首个缺失正数(只归位 [1,n])?         → 22 原地哈希
n+1 个值都在 [1,n],找重复且不改数组?       → 23 快慢指针
数组整体平移 k 位?                        → 24 三次反转
...
原始文档：[GitHub 源文件](https://github.com/keepkeen/llm-algo-job-notes/blob/main/%E7%AC%94%E8%AF%95/%E7%BA%AF%E5%8A%9B%E6%89%A3%E7%AE%97%E6%B3%95/%E5%9F%BA%E7%A1%80/Hot100%E8%A1%A5%E5%85%85%E7%AF%87%C2%B7%E4%BD%8D%E8%BF%90%E7%AE%97%E4%B8%8E%E6%8A%80%E5%B7%A7.md)。
