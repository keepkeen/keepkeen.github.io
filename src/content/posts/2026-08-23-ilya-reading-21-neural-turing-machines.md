---
title: "21. Neural Turing Machines：让神经网络学习可微的读、写与寻址"
description: "NTM 把 controller 与外部 N×M memory matrix 相连。读写头不选一个离散地址，而产生对所有地址的软权重；读、擦除、添加、内容查找和地址移动全部可微，因此仅凭输入输出样例就能用梯度下降学出复制、关联检索和排序的近似算法。"
date: 2026-08-23
tags:
  - deep-learning
  - paper-reading
...
[`src/ilya30/memory.py`](https://github.com/keepkeen/keepkeen.github.io/blob/main/public/code/ilya30/src/ilya30/memory.py) 实现 cosine content addressing、interpolation、circular shift、sharpen、read 和 erase-add write。单元测试覆盖权重归一、one-hot 精确读写、全擦除和移位回绕；这些是核心可复用算子，不伪装成论文所有任务的训练复现。
