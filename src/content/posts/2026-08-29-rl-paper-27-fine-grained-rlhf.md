...
- **冻结部分：** 多个奖励模型 $R_{\phi_k}$ 和初始参考策略 $P_{\theta_{\mathrm{init}}}$；PPO 期间不更新奖励模型。
- **更新部分：** 策略 $P_\theta$ 与价值模型 $V_\psi$。
...
| $P_\theta,P_{\theta_{\mathrm{init}}}$ | 当前与冻结初始策略 | 条件分布 | PPO 学习 / SFT 初始化 |
| $\beta$ | KL 惩罚强度 | 正标量 | 超参数 |
...
（正文公式 (2)），其中 $\bar y_p$ 比 $\bar y_l$ 漏掉的信息更少。

### 5.5 一组小数字走完奖励

假设答案有两句话，token 时刻 1–3 和 4–6；事实模型在每句末 $(t=3,6)$ 给分，完整性模型在 $(t=6)$ 给分。取 $w_{\mathrm{fact}}=0.5,w_{\mathrm{comp}}=0.3,\beta=0.1$。
