---
title: "24. Minimum Description Length：把学习定义为寻找最短的“模型 + 数据”编码"
description: "MDL 选择能让数据获得最短无歧义描述的模型。简单模型编码短但残差长，复杂模型残差短但自身或其模型类代价大；最短总码长在欠拟合和过拟合之间自动权衡。refined MDL 用 universal code/NML 取代随意的参数小数位编码，使原则成为可比较的统计方法。"
date: 2026-08-23
tags:
  - deep-learning
  - paper-reading
...
[`src/ilya30/complexity.py`](https://github.com/keepkeen/keepkeen.github.io/blob/main/public/code/ilya30/src/ilya30/complexity.py) 实现小样本 Bernoulli NML normalizer、NML codelength、two-part code 与压缩代理。测试穷举所有二进制串验证 `P_NML` 总和为 1，并比较公平/未知偏置模型。它不会把 zlib 长度冒充严格 MDL；神经网络参数若用普通 float 文件大小，编码选择会淹没理论问题。
