...
| (pi_psi) | 滞后遮罩策略 | 条件分布 | 每 (mu) 步复制 (pi_	heta) |
| (R) | 任务/偏好奖励 | 标量 | 自动指标或奖励模型 |
| (eta) | KL 约束强度 | 正标量 | 动态调节 |
...
\begin{cases}
\dfrac{\pi_\theta(a\mid s)}{\sum_{b\in V^p_\psi(s)}\pi_\theta(b\mid s)},&a\in V^p_\psi(s),\$$
6pt]
0,&a\notin V^p_\psi(s).
\end{cases}
$$
...
第四个 token 虽被当前策略赋予 0.15，也暂时不能采到。若选中第一个 token，当前策略概率 0.40、参考模型概率 0.50，且 (eta=0.1)，KL 样本惩罚为
