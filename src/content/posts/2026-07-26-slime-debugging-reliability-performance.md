...
![slime-debugging-reliability-performance 架构图 1](./assets/slime-interview-guide/slime-debugging-reliability-performance-diagram-1.svg)
...
资源公式先看 [第 06 章](../slime-configuration-resources-weight-sync/)。placement group 无界等待时会每 30 秒报告总 GPU 和可用 GPU，见 [`_create_placement_group`](https://github.com/THUDM/slime/blob/3778dbf6d1a533ab478ecf5ddaa11449a47752b2/slime/ray/placement_group.py#L42)。
...
项目文档将当前容错重点描述为 rollout-engine health check、重启、重新更新参数与 debug replay，见 [容灾文档](https://github.com/THUDM/slime/blob/3778dbf6d1a533ab478ecf5ddaa11449a47752b2/docs/zh/advanced/fault-tolerance.md#当前覆盖范围)。面试时应明确说：**rollout 侧自愈 ≠ exactly-once 数据处理 ≠ trainer 自动容错 ≠ 全作业无损恢复。**
...
性能改动要用**相同 dump/相同有效 token 数**对照。仅比较每 step 秒数可能被 response 变短、过滤更多样本或 loss mask 变少“优化”。Profiling 工具入口见 [性能分析文档](https://github.com/THUDM/slime/blob/3778dbf6d1a533ab478ecf5ddaa11449a47752b2/docs/zh/developer_guide/profiling.md#3-使用自动化-profiling-工具)。

## 9. 排障决策树

![slime-debugging-reliability-performance 架构图 2](./assets/slime-interview-guide/slime-debugging-reliability-performance-diagram-2.svg)
