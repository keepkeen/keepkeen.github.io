...
> 检索截止：2026-09-08 北京时间凌晨（§15 为 8.30–9.08 增量；§14/§13 保留历史证据日期；今天尚未发生的考试不在范围内）
>
...
## 15. 2026-09-08 增量：九月笔试与 Agent 排障

### 15.1 招聘与流程

[窗口日志 §0](/blog/recruit-window-refresh-log/)记录本轮来源和证据边界：美团 9.01 高校企业公告新增近期场次与专项互斥规则；vivo FAQ 确认一周后的截止/研发笔试；拼多多官网正式批证据增强；Seed、百度 AIDU 为存量复核。淘宝闪购生成式岗是新发现的转载线索，阿里国际 Agentic RL 公告为实习。腾讯/华为未取得足够的新大模型岗位字段，不据此写“停止招聘”。

### 15.2 公开样本（不混合不同岗位和试卷）

| ID | 来源与日期 | 已读主帖内容 | 证据边界 |
|---|---|---|---|
| SEP-JD-01 | [京东算法笔试](https://www.nowcoder.com/discuss/925858764907872256)，2026-09-05 21:12 发布，标题“9月5日” | NumPy 逻辑回归，JSON 训练/测试输入、输出概率；另有直线运动题和 AI Coding | 亲历自述。运动题条件不全；AI Coding 无题面/评分细节，不把“8880”解释成正确率 |
| SEP-BY-01 | [字节 Agent 开发一面](https://www.nowcoder.com/discuss/925342611194286080)，2026-09-04 发布，标题“9.3” | Harness、长任务从 5 分钟变成 15–20 分钟的排查、工具参数可靠性、上下文压缩；给定数字集合构造严格小于 n 的最大数 | 亲历自述，**Agent 开发相邻岗**；面试年份未单写，按帖子日期归档，不计作后训练研究岗高频证据 |
| SEP-PDD-02 | [PDD 0906 AI Agent 研发岗](https://www.nowcoder.com/discuss/926228178081705984)，2026-09-06 21:40 发布 | 传送带箱子最少反转；和为 k 倍数的无序对；魔法边有次数上限且不能连续使用的最短路；两侧配对收益最大化 | 亲历回忆，明确 Agent **研发**岗；四道传统算法题，未提 AI Coding/可用 AI。最短路与 KM 是作者给出的解法提示 |
| SEP-PDD-01 | [拼多多 9.06 机考整理](https://www.nowcoder.com/discuss/926278529782747136)，2026-09-07 发布 | “彩门换向”与模数配对 | 第三方整理，前两题与 SEP-PDD-02 重合，作为旁证不重复计题 |
| SEP-MT-01 | [美团算法策略 9.01 整理](https://onefly.top/zero2Leetcode/04_real_interviews/meituan/algo-20260901/index.html)，自标场次 2026-09-01 | 站点库存 AI Coding，整数/容量/预算/ID 对齐约束；欧拉回路题 | 二手，无候选人原始链接/发布日期；[另一份同日整理](https://www.nowcoder.com/discuss/924825304768487424)称多云成本 HTTP 服务。不同卷或误配尚未确定，不合并为固定试卷 |

上表是本轮新增的跨岗位观察附录，不与旧账本的 89 组算法面经直接相加。拼多多两个来源重合，不能当两套试卷。题面完整程度决定可复现范围；看到新日期并不意味着整页推荐内容都发生在该日。

### 15.3 三项实际补课

1. **完整 ML 提交**：按[AI Coding 补丁](/blog/ml-ai-coding-patch/)新增的自拟协议，限时完成 NumPy 逻辑回归；检查 shape、稳定 sigmoid、有限概率和 JSON 输出。真实卷若指定初始化/正则/迭代次数必须遵循题面。
2. **算法边界**：受限数字构造练逐位贪心与回退；彩门换向保留原位置，能开就开，整趟无进展才判失败；不把两题硬称为力扣原题。
3. **可观测 Agent 答辩**：先复现同一输入与版本，对比模型调用、工具耗时、重试次数、循环步数、输入/输出 token 和排队；并行工具看关键路径而非把耗时全相加。定位后再选择减上下文、修工具、限重试或缓存，并复测任务成功率。

### 15.4 强化学习阅读增量

本轮加入三篇原始论文案例，详细讲解见[前沿章 §7](/blog/llm-rl-frontier-topics/)：[竞赛代码后训练](https://arxiv.org/abs/2609.02849)练训练/搜索归因；[文本简化 GRPO](https://arxiv.org/abs/2609.04823)练奖励漏洞与切分泄漏审计；[First Things First](https://arxiv.org/abs/2609.05224)练必须条件、偏好和无解子集评测。它们是新论文，尚未作为面试实录收录，79 张评分卡保持不变。
---
