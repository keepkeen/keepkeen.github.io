...
`LLMServerManager` 管理 vLLM/SGLang/TRT-LLM 服务副本（都以 async server 模式注册；SPMD sync rollout 已在 v0.7 移除）；AgentLoopWorker 接收 prompt，调用 server client，累积 response ids、逐轮 token、mask 和工具结果。单轮任务也是一个最简单的 agent loop。

多轮时不能最后把 messages 重新套一遍 chat template 代替真实 rollout token，因为模板、空格或特殊 token 的差异会使训练时 log-prob 对应的 token 序列不同于采样策略，破坏 on-policy 一致性。这就是官方文档强调的 token-in/token-out 原则。
...
- reward model：另一个模型对回答评分（判别式 RM 或生成式 GenRM）。
- 多种 score 的组合与 shaping，例如超长惩罚。

RewardManager/RewardLoop 最终把结果整理为 token-shaped score。纯 outcome reward 常写在最后一个有效 response token。GAE、ReMax、REINFORCE++ 通过时间递推或 reward-to-go 传播；GRPO/RLOO 则先汇总序列 reward，再把组相对标量广播到有效 action token，它们不提供细粒度 temporal credit assignment。RewardLoop 可以和 rollout 流式衔接；生成式 RM 还可配置 `full_determinism`（#7027）保证判分可复现。
...
计算前经常按有效序列长度重新平衡 batch（`_balance_batch`），避免某些 data-parallel rank 被长回答拖慢。

## 5. reward shaping 与 advantage

若使用 reward-side KL，先从 token score 中扣除 KL penalty。随后根据 `algorithm.adv_estimator` 进入 estimator。response mask 决定哪些 token 有效；padding 和 EOS 后内容不能进入统计。
...
异步模式不必每 step 全部等待，但需要模型版本号、`parameter_sync_step`（separate_async 默认 4）、replay buffer 的 `max_off_policy_threshold`（默认 8）和 `drop|wait` 策略共同控制陈旧度。
...
| `response_mask` | response 尾段中哪些 token 参与 RL reward/advantage/loss（1=模型生成，0=工具/环境返回或 padding） |
