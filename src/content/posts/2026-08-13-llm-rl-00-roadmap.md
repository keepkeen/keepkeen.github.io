---
title: "大模型强化学习从零到面试（00）：专题导读与岗位地图"
description: "效果算法、Reasoning RL、Agentic RL、RL Infra 四类岗位的差异，2026 实录考察深度与经典 RL→LLM 的概念映射。"
date: 2026-08-13
tags:
  - ai
  - llm
...
> 本文是《大模型强化学习：从零到面试》专题第 00 章的发布版，核验日期 2026-08-13。配套零依赖参考实现与自动测试托管在 GitHub 仓库 [llm-algo-job-notes](https://github.com/keepkeen/llm-algo-job-notes)。
...
7. **前沿方向已入题（第二轮扩检确认）**：多模态可验证奖励（Visual-RFT 类）、熵坍缩机制、"RLVR 是否扩展能力"开放题、异步 RL 系统与 RL Scaling 在头部团队面试中出现。CV/检测背景候选人重点读 [12 章](/blog/llm-rl-12-frontier/) §1——它是把你原领域指标（IoU/PRO/AUROC）直接变成 RL 奖励叙事的桥。
